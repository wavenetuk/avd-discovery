...
function Assert-ScriptIsReadOnly {
	...
	Assert-DiscoveryCommandPolicy -ScriptPath $PSCommandPath -DeniedCmdlets $deniedCmdlets
}
...
function Get-OutlookCachedModeDiscovery {
	...
	$loadedUsers = if ($script:IsSystemAccountMode) { @() } else { Get-DiscoveryLoadedUserProfiles }
...
function Get-OneDriveAndFolderRedirectionDiscovery {
	...
	$userMappedDriveStates = foreach ($user in $loadedUsers) {
		Get-DiscoveryUserMappedDriveState -Sid $user.Sid -AccountName $user.AccountName
	}
...
function Invoke-ArmRequest {
	...
	$commonParams = @{ Path = $Path; Method = $Method }
...
function Get-FSLogixAzureFilesShareUsage {
	...
	try {
		...
		if (-not $originalContext -or -not $originalContext.Account) {
			$authenticatedContext = Connect-RequestedAzAccount -CurrentContext $originalContext -SignInMessage 'Sign in to the Azure account to use for detailed FSLogix share usage stats.' -ShowSignInWindowHint
		}
		else {
...
function Get-PrimaryApplicationConfigPath {
	return Resolve-DiscoveryConfigPath -ScriptRoot $PSScriptRoot -FileName 'appExclusions.config.json'
}
...
function Get-PrimaryApplicationConfig {
	...
	$config = Read-DiscoveryJsonConfig -Path $ConfigPath
...
function Get-InstalledApplications {
	...
	$filter = if ($PrimaryApplicationsOnlyMode) { { param($application) Test-PrimaryApplication -Application $application } } else { $null }
	return Get-DiscoveryInstalledApplications -ApplicationFilter $filter
}
...
try {
	...
	$machineDetails = Get-DiscoveryMachineDetails
	...
	$antivirusDiscovery = Get-DiscoveryAntivirus -InstalledApplications $installedApps
	Write-CheckResult 'Success'

	Write-CheckStart 'Active Directory Dependencies'
	$adDependencyDiscovery = Get-DiscoveryAdDsDependencies
	...
	$languagePackDiscovery = Get-DiscoveryLanguagePacks
	Write-CheckResult 'Success'

	Write-CheckStart 'Universal Print'
	$universalPrintDiscovery = Get-DiscoveryUniversalPrint
	Write-CheckResult 'Success'

	Write-CheckStart 'Printers'
	$printerDiscovery = Get-DiscoveryPrinterInventory
	Write-CheckResult 'Success' "$($printerDiscovery.PrinterCount) printer(s) found"

	Write-CheckStart 'Time Source'
	$timeSourceDiscovery = Get-DiscoveryTimeSource
	Write-CheckResult 'Success'
	...
	$exportObject = [PSCustomObject]@{
		...
		Collection           = New-DiscoveryCollectionMetadata -Workload 'Avd' -ScannerMode 'Host' -Capabilities @('HostDiscovery', 'HtmlReport')
	...
	try {
		...
		if (-not $NoHtml.IsPresent) {
			$exportObject.HtmlGeneration = Invoke-OptionalHtmlReportGeneration -JsonPath $stagingJsonPath -ReportType $exportObject.ReportType -OutputPath $resolvedHtmlPath -ScriptRoot $PSScriptRoot
