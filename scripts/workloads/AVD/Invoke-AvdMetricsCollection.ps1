...
Uses the active Azure PowerShell context instead of prompting for a new Azure sign-in. Run
Connect-AzAccount and, if needed, Set-AzContext before starting the collection. The script
still restores the original context when it finishes.
...
$script:armCounts = New-DiscoveryArmCallTracker
...
function Invoke-ArmRequest {
	...
	$commonParams = @{ Path = $Path; Method = $Method; CallTracker = $script:armCounts }
	if ($PSBoundParameters.ContainsKey('Payload')) {
		$commonParams['Payload'] = $Payload
	}
	Invoke-DiscoveryArmRequest @commonParams
}
...
function Assert-ScriptIsReadOnly {
	...
	Assert-DiscoveryCommandPolicy -ScriptPath $PSCommandPath -AllowedAzCmdlets $allowedAzCmdlets
}
...
function Invoke-HostPoolLocalDiscovery {
	...
	$scriptUrl       = "$GitHubRawBaseUrl/scripts/workloads/AVD/Invoke-AvdSessionHostAudit.ps1"
	$configUrl       = "$GitHubRawBaseUrl/config/appExclusions.config.json"
	$commonModuleUrl = "$GitHubRawBaseUrl/scripts/common/Discovery.Common.psm1"
	$hostBaselineModuleUrl = "$GitHubRawBaseUrl/scripts/common/Discovery.HostBaseline.psm1"
	$adDsModuleUrl = "$GitHubRawBaseUrl/scripts/common/Discovery.ADDS.psm1"
	...
	if ($InlineLocalScript.IsPresent) {
		...
		$localConfigPath = Join-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) 'config\appExclusions.config.json'
		$localCommonModulePath = Join-Path $PSScriptRoot '..\..\common\Discovery.Common.psm1'
		$localHostBaselineModulePath = Join-Path $PSScriptRoot '..\..\common\Discovery.HostBaseline.psm1'
		$localAdDsModulePath = Join-Path $PSScriptRoot '..\..\common\Discovery.ADDS.psm1'
		...
		if (-not (Test-Path $localCommonModulePath)) { Write-Warning "[LocalDiscovery | $($Pool.Name)] common\Discovery.Common.psm1 not found at '$localCommonModulePath' — cannot inline."; return }
		if (-not (Test-Path $localHostBaselineModulePath)) { Write-Warning "[LocalDiscovery | $($Pool.Name)] common\Discovery.HostBaseline.psm1 not found at '$localHostBaselineModulePath' — cannot inline."; return }
		if (-not (Test-Path $localAdDsModulePath)) { Write-Warning "[LocalDiscovery | $($Pool.Name)] common\Discovery.ADDS.psm1 not found at '$localAdDsModulePath' — cannot inline."; return }
		...
		$commonModuleB64 = [Convert]::ToBase64String([System.IO.File]::ReadAllBytes($localCommonModulePath))
		$hostBaselineModuleB64 = [Convert]::ToBase64String([System.IO.File]::ReadAllBytes($localHostBaselineModulePath))
		$adDsModuleB64 = [Convert]::ToBase64String([System.IO.File]::ReadAllBytes($localAdDsModulePath))
		...
		$commonModuleAssignment = Split-Base64ToChunkedAssignment -VarName 'commonB64' -Base64 $commonModuleB64
		$hostBaselineModuleAssignment = Split-Base64ToChunkedAssignment -VarName 'hostBaselineB64' -Base64 $hostBaselineModuleB64
		$adDsModuleAssignment = Split-Base64ToChunkedAssignment -VarName 'adDsB64' -Base64 $adDsModuleB64
		...
		$wrapperLines = @(
			$scrAssignment,
			$cfgAssignment,
			$commonModuleAssignment,
			$hostBaselineModuleAssignment,
			$adDsModuleAssignment,
			...
			'    $commonDir = Join-Path $tmp "common"',
			'    $commonPath = Join-Path $commonDir "Discovery.Common.psm1"',
			'    $hostBaselinePath = Join-Path $commonDir "Discovery.HostBaseline.psm1"',
			'    $adDsPath = Join-Path $commonDir "Discovery.ADDS.psm1"',
			...
			'    [System.IO.File]::WriteAllBytes($commonPath, [Convert]::FromBase64String($commonB64))',
			'    [System.IO.File]::WriteAllBytes($hostBaselinePath, [Convert]::FromBase64String($hostBaselineB64))',
			'    [System.IO.File]::WriteAllBytes($adDsPath, [Convert]::FromBase64String($adDsB64))',
		...
		$wrapperLines = @(
			"`$scriptUrl = '$scriptUrl'",
			"`$configUrl = '$configUrl'",
			"`$commonModuleUrl = '$commonModuleUrl'",
			"`$hostBaselineModuleUrl = '$hostBaselineModuleUrl'",
			"`$adDsModuleUrl = '$adDsModuleUrl'",
			"`$cCode     = '$custCodeEscaped'",
			'$ErrorActionPreference = "Stop"',
			'try {',
			...
			'    Invoke-WebRequest -Uri $commonModuleUrl -OutFile $commonPath -UseBasicParsing',
			'    Invoke-WebRequest -Uri $hostBaselineModuleUrl -OutFile $hostBaselinePath -UseBasicParsing',
			'    Invoke-WebRequest -Uri $adDsModuleUrl -OutFile $adDsPath -UseBasicParsing',
			("    & `$scrPath -CustomerAbbreviation `$cCode -OutputDirectory `$outDir -PrimaryApplicationsOnly -NoHtml$(if ($NoGpresult.IsPresent) { ' -NoGpresult' }) -ErrorAction Stop *>&1 | Out-Null"),
			'    $jf = Get-ChildItem -Path $outDir -Filter "*.json" -File | Select-Object -First 1',
			'    if (-not $jf) { throw "No JSON output produced by Invoke-AvdSessionHostAudit.ps1" }',
			'    $jb  = [System.IO.File]::ReadAllBytes($jf.FullName)',
	...
	try {
		...
		$parsedLocalDiscovery.HtmlGeneration = if ($NoHtml.IsPresent) {
			...
			Invoke-OptionalHtmlReportGeneration -JsonPath $outputFile -ReportType 'AzureSessionHostAudit' -OutputPath ([System.IO.Path]::ChangeExtension($outputFile, '.html')) -ScriptRoot $PSScriptRoot
		}
...
try {
	...
	if ($UseCurrentContext) {
		if (-not $originalContext -or -not $originalContext.Account -or [string]::IsNullOrWhiteSpace([string]$originalContext.Account.Id)) {
			throw 'No active Azure context is available. Run Connect-AzAccount first, or run without -UseCurrentContext to sign in.'
		}
		$authenticatedContext = $originalContext
		Write-Host "  Using current Azure context: $($authenticatedContext.Account.Id)" -ForegroundColor DarkGray
	}
	else {
		$authenticatedContext = Connect-RequestedAzAccount -CurrentContext $originalContext -PromptForAccountId
	}
	...
	$GeneratedBy   = if ([string]::IsNullOrWhiteSpace($GeneratedBy)) { $null } else { $GeneratedBy.Trim() }
	$ProjectCode   = if ([string]::IsNullOrWhiteSpace($ProjectCode)) { $null } else { $ProjectCode.Trim() }
	$endTime       = (Get-Date).ToUniversalTime().Date          # midnight UTC today
	$startTime     = $endTime.AddDays(-$LookbackDays)

	$resolvedOutputDirectory    = if ([string]::IsNullOrWhiteSpace($OutputDirectory)) { Join-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) 'output\avd-metrics' } else { [System.IO.Path]::GetFullPath($OutputDirectory) }
	$resolvedOutputPath         = New-ExportFilePath -Directory $resolvedOutputDirectory -CustomerCode $customerCode
	$resolvedVmDiscoveryDirectory = Join-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) 'output\vm-discovery'
	...
	$skuDisplayNameMap = @{}
	$skuCsvPath = Join-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) 'config\ms-service-plan-ids.csv'
	...
	$exportObject = [PSCustomObject]@{
		...
		Collection              = New-DiscoveryCollectionMetadata -Workload 'Avd' -ScannerMode 'Cloud' -Capabilities @('ManagementPlane', 'UsageMetrics', 'HtmlReport')
	...
	if (-not $NoHtml.IsPresent) {
		$exportObject.HtmlGeneration = Invoke-OptionalHtmlReportGeneration -JsonPath $resolvedOutputPath -ReportType $exportObject.ReportType -OutputPath $resolvedHtmlPath -ScriptRoot $PSScriptRoot
	}
	$exportObject | ConvertTo-Json -Depth 8 | Set-Content -Path $resolvedOutputPath -Encoding UTF8
